...
public class PdfPTable implements LargeElement{
    ...
    public PdfPTable(PdfPTable table) {
        ...
        for (int k = 0; k < table.rows.size(); ++k) {
            ...
            if (row != null) row = new PdfPRow(row);
            rows.add(row);
        }
    }
    ...
    private void calculateWidths() {
      ...
      //Update relative widths using column widths defined in the first row of the table
        {

            java.util.HashMap<Integer, Float> relWidths = new java.util.HashMap<Integer, Float>();
            java.util.HashMap<Integer, Float> fixedWidths = new java.util.HashMap<Integer, Float>();

            PdfPRow row = (PdfPRow) rows.get(0);
            PdfPCell[] cells = row.getCells();
            for (int i=0; i<cells.length; i++) {
                PdfPCell cell = cells[i];
                int colIndex = i; //what about col span?


                String width = cell.getColumnWidth();
                if (width!=null){
                    if (width.endsWith("%")){
                        try{
                            float p = Float.parseFloat(width.substring(0, width.length()-1))/100f;
                            relWidths.put(i, p);
                        }
                        catch(Exception e){
                        }
                        ...
                        try{
                            float p = Float.parseFloat(width.substring(0, width.length()-2))/totalWidth;
                            fixedWidths.put(i, p);
                        }
                        catch(Exception e){
                        }
          ...
          //Update relativeWidths using column widths defined in percentages
            float totalPercent = 0;
            float[] widths = new float[relativeWidths.length];
            for (int i=0; i<widths.length; i++){
                Float p = relWidths.get(i);
                if (p==null) p = 0.01f;
                widths[i] = p;
                totalPercent+=p;
            }
            if (totalPercent>1.0f){
                float scale = 1.0f/totalPercent;
                totalPercent = 0f;
                for (int i=0; i<widths.length; i++){
                    if (widths[i]>0) widths[i] = widths[i]*scale;
                    totalPercent+=widths[i];
                }
            }
            if (totalPercent<1.0f){
                int numAdjustments = widths.length - relWidths.size();
                float totalAdjustment = 1.0f-totalPercent;
                for (int i=0; i<widths.length; i++){
                    if (!relWidths.containsKey(i)){
                        widths[i] += totalAdjustment/numAdjustments;
                    }
                }
            }


          //Update column widths using column widths defined in pixels. These
          //widths take precendence over widths defined as percentages.
            totalPercent = 0;
            for (int i=0; i<widths.length; i++){
                Float p = fixedWidths.get(i);
                if (p!=null) widths[i] = p;
                totalPercent+=widths[i];
            }
            if (totalPercent>1.0f){
                float scale = 1.0f/totalPercent;
          ...
          //Update relativeWidths
            for (int i=0; i<widths.length; i++){
                float p = widths[i];
                relativeWidths[i] = (relativeWidths[i]*total)*p;
            }
        }
      ...
      //Check if there are any columns that have a "nowrap" style. If so, check
      //if any of these columns exceed their absoluteWidth value.
        boolean updateWidth = false;
        java.util.HashSet<Integer> fixedCols = new java.util.HashSet<Integer>();
        java.util.HashMap<Integer, Float> colWidths = new java.util.HashMap<Integer, Float>();
        for (int i=0; i<rows.size(); i++) {
            ...
            for (int j=0; j<cells.length; j++) {
                ...
                float padding = cell.getPaddingLeft() + cell.getPaddingRight();
                int colIndex = j; //what about col span?
                boolean fixedWidth = cell.isNoWrap();
                if (fixedWidth) fixedCols.add(colIndex);

              //Compute content width for each cell
                LinkedList<Element> list = (LinkedList<Element>) cell.getCompositeElements();
                if (list!=null){
                    java.util.Iterator<Element> it = list.iterator();
                    while (it.hasNext()){
                        Element el = it.next();
                        ArrayList<com.lowagie.text.Chunk> chunks = (ArrayList<com.lowagie.text.Chunk>) el.getChunks();
                        if (chunks!=null){
                            for (com.lowagie.text.Chunk chunk : chunks){
                                float width = chunk.getWidthPoint() + padding;
                                Float maxWidth = colWidths.get(colIndex);
                                if (maxWidth==null) colWidths.put(colIndex, width);
                                else colWidths.put(colIndex, Math.max(width, maxWidth));
                            }
                        }
                    }
                }
              ...
              //Update the "updateWidth" variable as needed
                if (fixedWidth){
                    Float maxWidth = colWidths.get(colIndex);
                    if (maxWidth!=null){
                        if (maxWidth>absoluteWidths[colIndex]) updateWidth = true;
                    }
                }
            }
        }
      ...
      //If there are columns that exceed their absoluteWidths, make adjustments
        if (updateWidth){

            java.util.ArrayList<Integer> adjustableColumns = new java.util.ArrayList<Integer>();
            for (int i=0; i<numCols; i++) adjustableColumns.add(i);

            int iterations = 0;
            while (true){


              //Compute excess width
                float excessWidth = 0f;
                for (int i=0; i<numCols; i++){
                    boolean fixedWidth = fixedCols.contains(i);
                    Float maxWidth = colWidths.get(i);
                    if (maxWidth!=null){
                        if (maxWidth>absoluteWidths[i] && fixedWidth) {
                            float excess = maxWidth-absoluteWidths[i];
                            excessWidth+=excess;
                            absoluteWidths[i] = maxWidth;
                            try{adjustableColumns.remove(i);}catch(Exception e){}
                        }
                    }
                }
              ...
              //Distribute the excess width across other columns
                distributeWidth(excessWidth, adjustableColumns, fixedCols, colWidths);
      ...
      //If the width of all the columns exceed the totalWidth of the table, make adjustments
        {
            java.util.ArrayList<Integer> adjustableColumns = new java.util.ArrayList<Integer>();
            for (int i=0; i<numCols; i++) adjustableColumns.add(i);
            ...
            while (true){
              ...
              //Compute excess width
                float currTableWidth = 0;
                for (int i=0; i<numCols; i++){
                    currTableWidth+=absoluteWidths[i];
                }
                float excessWidth = currTableWidth-totalWidth;
                if (excessWidth==0) break;
  ...
  /** Used to distribute excess pixels by adjusting column widths
   */
    private void distributeWidth(float excessWidth, java.util.ArrayList<Integer> adjustableColumns,
        java.util.HashSet<Integer> fixedCols, java.util.HashMap<Integer, Float> colWidths){

        int numAdjustments = adjustableColumns.size();
        float x = excessWidth/(float) numAdjustments;
        //System.out.println("Distribute " + excessWidth + " across " + numAdjustments + " columns out of " + numCols);
        //System.out.println(adjustableColumns);

        for (int i: adjustableColumns){
            float colWidth = absoluteWidths[i];
            float newWidth = colWidth-x;
            boolean fixedWidth = fixedCols.contains(i);
            Float maxWidth = colWidths.get(i);
            if (maxWidth==null){
                absoluteWidths[i] = newWidth;
            }
            else{
                if (newWidth<maxWidth && fixedWidth){
                    //System.out.println("Skipping " + i + "...");
                    continue;
                }
                //System.out.println("Updating " + i + " from " + colWidth + " to " + newWidth + "...");
                absoluteWidths[i] = newWidth;
     ...
     * This takes time; normally the heights of the rows are already calculated,
    ...
    public float writeSelectedRows(int colStart, int colEnd, int rowStart, int rowEnd, float xPos, float yPos, PdfContentByte canvas) {
        ...
        else colStart = Math.min(colStart, totalCols);

    	if (colEnd < 0) colEnd = totalCols;
    	else colEnd = Math.min(colEnd, totalCols);
    ...
    public float getRowHeight(int idx, boolean firsttime) {
        ...
        if (firsttime) row.setWidths(absoluteWidths);

        float height = row.getMaxHeights();

        for (int i = 0; i < relativeWidths.length; i++) {
            if (!rowSpanAbove(idx, i)) continue;
            int rs = 1;
