 ...
 */
    ...
    public boolean newPage() {
        try {
            ...
            write("=\"");
            writeCssProperty(HtmlTags.CSS_KEY_PAGE_BREAK_BEFORE, HtmlTags.CSS_VALUE_ALWAYS);
            write("\" /");
            os.write(GT);
        }
        catch(IOException ioe) {
        ...
        try {
            switch(element.type()) {
                ...
                case Element.TITLE:
                    ...
                    os.write(GT);
                    addTabs(3);
                    write(encode(((Meta)element).getContent()));
                    addTabs(2);
                    writeEnd(HtmlTags.TITLE);
                    return true;
                case Element.CREATOR:
                    writeComment("Creator: " + encode(((Meta)element).getContent()));
                    return true;
                case Element.PRODUCER:
                    writeComment("Producer: " + encode(((Meta)element).getContent()));
                    return true;
                case Element.CREATIONDATE:
                    writeComment("Creationdate: " + encode(((Meta)element).getContent()));
                    return true;
                case Element.MARKED:
    ...
    public void open() {
        ...
        try {
            ...
            if (pageSize.getBackgroundColor() != null) {
                write(HtmlTags.BACKGROUNDCOLOR, encode(pageSize.getBackgroundColor()));
            }
            if (document.getJavaScript_onLoad() != null) {
                write(HtmlTags.JAVASCRIPT_ONLOAD, encode(document.getJavaScript_onLoad()));
            }
            if (document.getJavaScript_onUnLoad() != null) {
                write(HtmlTags.JAVASCRIPT_ONUNLOAD, encode(document.getJavaScript_onUnLoad()));
            }
            if (document.getHtmlStyleClass() != null) {
                write(HtmlTags.HTML_ATTR_CSS_CLASS, document.getHtmlStyleClass());
            }
            os.write(GT);
 ...
 */
    ...
    protected void writeHeader(Meta meta) throws IOException {
        ...
        write(HtmlTags.CONTENT, encode(meta.getContent()));
        writeEnd();
    }
 ...
 */
    ...
    protected void writeJavaScript(Header header) throws IOException {
        ...
        if (markup.size() > 0) {
           ...
           * </script>
           */ 
          write(HtmlTags.TYPE, HtmlTags.HTML_VALUE_JAVASCRIPT);
          os.write(GT);
          addTabs(2);
    ...
    protected void write(Element element, int indent) throws IOException {
        ...
        switch(element.type()) {
            ...
            case Element.CHUNK:
            {
                ...
                if (tag) {
                    // end tag
                    writeEnd(HtmlTags.HTML_TAG_SPAN);
                }
                return;
            }
            case Element.PHRASE:
            {
                ...
                if (phrase.hasLeading()) styleAttributes.setProperty(HtmlTags.CSS_KEY_LINEHEIGHT, phrase.getLeading() + "pt");
                
                // start tag
                addTabs(indent);
                writeStart(HtmlTags.HTML_TAG_SPAN);
                writeMarkupAttributes(markup);
            ...
            case Element.ANCHOR:
            {
                ...
                styleAttributes = new Properties();
                if (anchor.hasLeading()) styleAttributes.setProperty(HtmlTags.CSS_KEY_LINEHEIGHT, anchor.getLeading() + "pt");
            ...
            case Element.PARAGRAPH:
            {
                ...
                styleAttributes = new Properties();
                if (paragraph.hasLeading()) styleAttributes.setProperty(HtmlTags.CSS_KEY_LINEHEIGHT, paragraph.getTotalLeading() + "pt");
                // start tag
                addTabs(indent);
                writeStart(HtmlTags.DIV);
                writeMarkupAttributes(markup);
            ...
            case Element.LISTITEM:
            {
                ...
                styleAttributes = new Properties();
                if (listItem.hasLeading()) styleAttributes.setProperty(HtmlTags.CSS_KEY_LINEHEIGHT, listItem.getTotalLeading() + "pt");
            ...
            case Element.CELL:
            {
                ...
                if (cell.getBorderColor() != null) {
                    write(HtmlTags.BORDERCOLOR, encode(cell.getBorderColor()));
                }
                if (cell.getBackgroundColor() != null) {
                    write(HtmlTags.BACKGROUNDCOLOR, encode(cell.getBackgroundColor()));
                }
                String alignment = getAlignment(cell.getHorizontalAlignment());
                if (!"".equals(alignment)) {
                    write(HtmlTags.HORIZONTALALIGN, alignment);
                }
                alignment = getAlignment(cell.getVerticalAlignment());
                if (!"".equals(alignment)) {
            ...
            case Element.TABLE:
            {
                ...
                os.write(QUOTE);
                String alignment = getAlignment(table.getAlignment());
                if (!"".equals(alignment)) {
                    write(HtmlTags.ALIGN, alignment);
                }
                ...
                if (table.getBorderColor() != null) {
                    write(HtmlTags.BORDERCOLOR, encode(table.getBorderColor()));
                }
                if (table.getBackgroundColor() != null) {
                    write(HtmlTags.BACKGROUNDCOLOR, encode(table.getBackgroundColor()));
                }
                os.write(GT);
                // contents
                Row row;
 ...
 */
    ...
    protected void writeSection(Section section, int indent) throws IOException {
        if (section.getTitle() != null) {
            ...
            if (section.getTitle().hasLeading()) styleAttributes.setProperty(HtmlTags.CSS_KEY_LINEHEIGHT, section.getTitle().getTotalLeading() + "pt");
            // start tag
            addTabs(indent);
            writeStart(HtmlTags.H[depth]);
            write(section.getTitle().getFont(), styleAttributes);
            String alignment = getAlignment(section.getTitle().getAlignment());
            if (!"".equals(alignment)) {
                write(HtmlTags.ALIGN, alignment);
            }
            writeMarkupAttributes(markup);
    ...
    protected void write(Font font, Properties styleAttributes) throws IOException {
        ...
        if (isOtherFont(font)) {
            writeCssProperty(HtmlTags.CSS_KEY_FONTFAMILY, font.getFamilyname());
            
            if (font.getSize() != Font.UNDEFINED) {
                writeCssProperty(HtmlTags.CSS_KEY_FONTSIZE, font.getSize() + "pt");
            }
            if (font.getColor() != null) {
                writeCssProperty(HtmlTags.CSS_KEY_COLOR, encode(font.getColor()));
            }
            ...
            if (fontstyle != Font.UNDEFINED && fontstyle != Font.NORMAL) {
                switch (fontstyle & Font.BOLDITALIC) {
                    case Font.BOLD:
                        writeCssProperty(HtmlTags.CSS_KEY_FONTWEIGHT, HtmlTags.CSS_VALUE_BOLD);
                        break;
                    case Font.ITALIC:
                        writeCssProperty(HtmlTags.CSS_KEY_FONTSTYLE, HtmlTags.CSS_VALUE_ITALIC);
                        break;
                ...
                if ((fontstyle & Font.UNDERLINE) > 0) {
                    writeCssProperty(HtmlTags.CSS_KEY_TEXTDECORATION, HtmlTags.CSS_VALUE_UNDERLINE);
                }
                if ((fontstyle & Font.STRIKETHRU) > 0) {
                    writeCssProperty(HtmlTags.CSS_KEY_TEXTDECORATION, HtmlTags.CSS_VALUE_LINETHROUGH);
                }
            }
        }
        write("\"");
 ...
 */
    ...
    private String encode(java.awt.Color color) {
        StringBuffer buffer = new StringBuffer("#");
        if (color.getRed() < 16) {
            buffer.append('0');
        }
