...
public class Section extends ArrayList implements TextElementArray, LargeElement {
    ...
    public Section(Paragraph title) {
        this(title, 0);
    ...
    public static Paragraph constructTitle(Paragraph title, ArrayList numbers, int numberDepth, int numberStyle) {
        ...
        int numSize = numbers==null? 0 : numbers.size();
        int depth = Math.min(numSize, numberDepth);
        if (depth < 1) {
            return title;
        }
     ...
     */
    public int getDepth() {
        return numbers==null? 0 : numbers.size(); //numbers.size();
    }
    ...
    private void setNumbers(int number, ArrayList numbers) {
        ...
        if (numbers!=null) this.numbers.addAll(numbers);
    }
